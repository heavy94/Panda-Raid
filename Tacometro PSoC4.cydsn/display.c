...
void display_update(uint32 speed, uint32 heading, uint32 rpm)
{
    ...
    uint8 seg_ten = 0;
    uint8 seg_uni = 0;
    uint32 seg_rpm = 0; //Rightmost value represent de first LED
    ...
    tx_array[2] = (B_MASK(seg_uni) << 7) | (G_MASK(seg_uni) << 6) | (F_MASK(seg_ten) << 5) | (A_MASK(seg_cen) << 4) |
                  (C_MASK(seg_cen) << 3) | (D_MASK(seg_ten) << 2) | (E_MASK(seg_uni) << 1);
    ...
    tx_array[6] = (E_MASK(seg_cen) << 7) | (C_MASK(seg_cen) << 6) | (A_MASK(seg_ten) << 5) | (D_MASK(seg_ten) << 4) |
                  (B_MASK(seg_uni) << 2) | (E_MASK(seg_uni) << 1) | C_MASK(seg_ten);
